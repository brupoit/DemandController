...
const columns = [
    { label: 'Nome', fieldName: 'InfoId', type: 'url',
    typeAttributes:{
  ...
  label:{fieldName: 'perfil'}
    ...
    { label: 'Horas Alocadas', fieldName: 'HorasAlocadas', type:'number', sortable:true },
];

export default class RecursosAlocadosControl extends LightningElement {
    columns=columns;
    data;
    ...
    connectedCallback(){
        getAllProfissionalComDadosDeHoras({strDataInicio:'2021-12-01',strDataFim:'2021-12-31'}).then(result=>{
          ...
          parsedResult.forEach((element)=>{
            ...
            addRow.HorasAlocadas = getNullOrRecordResult(element.horasAlocadas);
