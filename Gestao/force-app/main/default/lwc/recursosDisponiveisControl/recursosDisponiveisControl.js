...
{ label: 'Nome', fieldName: 'InfoId', type: 'url', sortable:true ,
  ...
  { label: 'Horas Disponiveis', fieldName: 'horasdisponiveis', type:'number', sortable:true},
     
];


export default class RecursosDisponiveisControl extends LightningElement {
  columns=columns;
    data;
    url = window.location.href.substring(0, window.location.href.indexOf('/lightning/'));
    connectedCallback(){
      getAllProfissionalComDadosDeHoras({strDataInicio:'2021-12-01',strDataFim:'2022-01-31'}).then(result=>{
            ...
            parsedResult.forEach((element)=>{
              ...
              addRow.horasdisponiveis = getNullOrRecordResult(element.horasDisponíveis);
